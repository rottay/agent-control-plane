...
| P-14 | Bootstrap mínimo: iniciativa y tarea por una puerta real | **Entregado 2026-09-13 (commits A `1418e58`, B `f7cd466`, C `c85eeb8`; ADR 0085–0087; decisiones 70–79).** Deuda nombrada: la existencia del paso dentro del documento del roadmap era de P-26 (ADR 0087, decisión 78), pasó a P-27 por la decisión 193 (implementable desde P-26/B, no pagada al aceptar P-26) y **la paga P-27/A** (ADR 0115, decisión 197: `ROADMAP_STEP_UNKNOWN` y `ROADMAP_STEPS_UNDECLARED`); la ejecución del ingreso y la enmienda al ADR 0080 §4 son de P-15; el preflight de `authority[]` es de P-23; las particiones INITIATIVE/STEP son de P-28. **Errata (2026-09-23):** el bootstrap del registry que el párrafo bajo §1 pone en P-14 no se entregó ni se nombró como deuda; lo paga P-15/R (ADR 0104, decisión 127). Diseño: §5. | [contratos §5](../../architecture/contracts/index.md), hoja de planificación | P-05, P-09/log, P-36/local | operativa: rutas contra HEAD |
| P-15 | Composición real de los clientes de API y local, y lectura del resultado | **Entregado 2026-09-26 (commits A `313512d`, B `a0584dd`, C `64b1dfd`, R `fc1e0d1`, D1 `b289b17`, D2 `be3b06f`, D3 `666537c`, D4 `c1bb414`, I `cb81f37`, F0 `8515047`, F `fd84256`, E `a1a8416`, G `1a745f8`, A2 `a804f2f`, A3 `6ea5f7d` y A4 `219afa9`; ADR 0101–0108, 0112, 0114 y 0119; decisiones 116–160, 178–183, 189–192 y 214–216; cada corte con receipt independiente), aceptado por decisión 217 el 2026-09-26 en el límite del packet.** M2 (decisión 218): el smoke S1 intento 4, una invocación bajo la autorización del owner del 2026-09-25 (ítem 2), entró por `acp intake`, corrió en el daemon registrado sobre el CLI 2.1.281 fijado con Haiku 4.5 por `CLI_SUBSCRIPTION` y cuota incluida, y se leyó por `acp effects` y `acp result`: `RESULT` `SUCCEEDED`, uso CUMULATIVE final sólo de `result.usage` y la entrega fijada a `catalog-s1-cli-subscription` v1; B5 y B15 cierran para M2 y N02 se cierra; el perfil de M2 anuncia un proveedor, Claude por `CLI_SUBSCRIPTION` —`API_KEY` y `LOCAL_OR_SELF_HOSTED` quedan aceptados en código por puertas reales con hijos sintéticos (D-F-4, D-F-5) y no se anuncian como confirmados por smoke—; M2 sigue abierto hasta que A1 (P-14) y B1 (P-05) cierren por su propio acto de documentación, con el re-sellado de la fila de P-05. Lo que P-15 deja (decisión 219): ND-A2-7, las filas negativas `dontAsk`/`auto`/`manual` del límite de la decisión 214, una captura de 2.1.282 o posterior si cambia el binario fijado, y `--permission-prompts none`/`--disallowedTools` sólo con evidencia nueva → P-23; D-S1-2 y el retiro de `objective` → P-16; D-S1-3 y D-S1-4 → P-18; `allowed_warning` como presión, la ceguera de cuota de los walks V2 y el resolver más allá de `file://` → P-19; la reconciliación por handle → M4 (P-18); la propuesta P1 (ADR 0108, decisión 155), residual condicional al owner, que el corte de documentación siguiente nombra en la fila de su dueño; E2 y E3 (M7) y E5 (M14), con S2 y S3 sólo bajo una nueva autorización del owner. El intento de S1 que el ítem 2 aún autorizaba caduca con esta aceptación. Registro de los cortes: **escalones A `313512d`, B `a0584dd`, C `64b1dfd`, R `fc1e0d1`, D1 `b289b17`, D2 `be3b06f`, D3 `666537c`, D4 `c1bb414`, I `cb81f37` y F0 `8515047`, F `fd84256` y E `a1a8416` (los clientes reales de API y local, y el resolver mínimo de credenciales) entregados; ADR 0101–0108; decisiones 116–160.** **A2 entregado (ADR 0112; decisiones 178–183):** el adapter Claude admite los streams 2.1.280 y 2.1.281 por forma observada, rechaza una versión no observada con `PROTOCOL_UNSUPPORTED` en su primer registro —después del spawn, sin impedir gasto; la compuerta previa al spawn es ND-A2-7, re-alojada a P-23 por la decisión 219— y guarda la versión en el cursor. **A3 (ADR 0114, enmienda al ADR 0112; decisiones 189–192):** el reintento de S1 (G, 2026-09-24, una invocación autorizada, gastada) falló cerrado porque la tabla de A2 indexaba `rate_limit_event` sólo por versión y rechazó con `UNKNOWN_EVENT` su registro 6 (2.1.281, `status: "allowed"`, conjunto de claves de overage); la tabla se indexa ahora por cada campo en que las capturas muestran variar la forma — `rate_limit_event` por (versión, status), tres formas observadas, cada una citando su captura, todo otro par rechazado (revierte ND-A2-9) — con la muestra 4 (el stream del reintento saneado, `21a6d56e…5749`) y los drills T-G2 (puerto) y T-D2 (daemon, clase sin terminal), más T-D3, compuesto, ninguna captura lo muestra; sin cambio del kit: su segundo fixture y la re-fijación son del operador de S1 con nueva autorización del owner. **El intento 3 de S1 falló (G, 2026-09-25):** `READ_ONLY_VIOLATION`, un `Write` del archivo de plan emitido por el revisor en modo plan, matado por la capa 2; el parser admitió los 11 registros. **A4 (ADR 0119, enmienda al ADR 0101; decisiones 214–216):** el argv del revisor sin modo plan (`--restricted --tools <allowlist>`, sin `--permission-mode`; capas 1 y 2 sin cambio); la muestra 5 (el stream del intento 3 saneado, `476aba4f…e0a5`); T-G3 con sus controles de rol y de nombre de herramienta, T-V1 (la tabla de veredictos cerrada por captura y rol), T-D4 (la clase del intento 3 por las puertas como revisor) y T-D5 (compuesto, ninguna captura lo muestra), ambos leyendo el argv que recibió el hijo; sin cambio del kit. **G tras A4:** el intento 4 de S1 (2026-09-26) pasó (decisión 217); el retiro de `objective` va con el primer bump de contrato de P-16, dueño P-16. **S1 falló (G, 2026-09-24; D-S1-1, decisión 160):** con el CLI 2.1.281 el parser del adapter Claude rechazó con `UNKNOWN_EVENT` el registro `system/thinking_tokens` (registro 1, tras `init`); la sesión se mató y la tarea quedó `FAILED`, `NO_RESULT_RECORDED`, con el uso `UNKNOWN`. **Obligación de A2:** el adapter admite el stream 2.1.281 por forma observada —una tabla de registros sin señal, con conjunto de claves estricto y compuertas por campo, sólo con los valores que una captura muestra, y `estimated_tokens` fuera del uso— sobre la captura completa que el owner autorizó y que se tomó el 2026-09-24 (CLI 2.1.281, saneada `a1bd7d82…095a`); después, una nueva corrida de S1 con su propia autorización: el reintento (decisión 189), el intento 3 (decisión 214) y el intento 4, verde (decisión 217). **Antes de la nueva corrida de S1 (A2, decisión 183; artefactos del operador):** la cuarta corrección de `s1-assert` (Fable C5: `isUsingOverage === false` siempre; `overageStatus` se afirma sólo donde la clave existe — errata de A3, decisión 192: 2.1.281 sí emite `overageStatus: "rejected"` bajo `allowed`, y no bajo `allowed_warning`), el hijo del ensayo reconstruido desde la muestra 2.1.281 y la auto-actualización del CLI como condición de parada; cumplido en el kit del operador (`G/attempt-4/prep.md`). **Obligación de E (P-15/F, ADR 0107, decisión 154):** los drills D-F-4 (`API_KEY`, el `fetch` sustituto de E reproduciendo un stream de Messages grabado) y D-F-5 (`LOCAL_OR_SELF_HOSTED`, el stream falso compatible con OpenAI de E) entran por una puerta real y leen de vuelta por `acp result` y el GET privado `taskEffectResult`; E no es aceptable sin esas dos filas. **E las entrega (ADR 0108, decisión 159):** D-F-4 por `POST /tasks` sobre el sustituto de `fetch` (SOCKET_EXERCISED: NONE) y D-F-5 por `acp intake` sobre un socket de loopback real (fixture `.mjs` admitido), con `CREDENTIAL` y `NONE`, leídos por las dos puertas y con el canario barrido de todos los sumideros. **Autorización del owner C4 y E-ND-1:** sólo implementación y pruebas sintéticas; ninguna llamada a una API comercial ni smoke S2. **B15 en dos etapas:** F cierra su aceptación de código; la fila del registro cambia en M2 con G/S1 o un fallo explícito del DT; cambió con el intento 4 de S1 (decisión 218) | [contratos §4](../../architecture/contracts/index.md), [arquitectura §6](../../architecture/index.md) | P-06, P-07, P-14, P-13; antes de gasto: P-32/captura, P-33/catalogo | operativa: rutas |

### 1.3 M3–M4 — verificación, commit, recuperación y ownership

| ID | Packet | Diseño | Contratos y schemas | Depende de | Falta congelar |
| --- | --- | --- | --- | --- | --- |
| P-16 | Verificador y auditor ejecutados, con receipt completo | `DESIGN_READY`; **corte A1 (ADR 0120; decisiones 220–224; receipt independiente y `pnpm check` completo antes del commit): `TaskEnvelope` sin `objective`, contrato 2.11.0 (diez versiones leíbles), API 0.25.0 sin ruta nueva, y D-B-1 por nombre antes de todo gasto.** Quedan, de este paquete, el receipt v2, la verificación y la auditoría ejecutadas (A2, B, C, D1–D2) y D-S1-2 | [contratos §12](../../architecture/contracts/index.md), [base de datos §6.6](../../architecture/database/index.md) | P-15, P-18/protocolo, P-36/local | operativa: rutas. **Obligación heredada, pagada por P-16/A1 (decisión 220):** retiro de `objective` del envelope con el primer bump de contrato de P-16 (re-apuntado por P-15/D4, y sin versión fija por la decisión 160: si otro paquete sube antes `CONTRACT_VERSION`, la obligación sigue al primer bump de P-16; mueve los vectores de envelope_sha256, decisión 48; el write-set de ese corte incluye task-envelope). **Obligación heredada de S1 (D-S1-2, decisión 160):** la forma registrada del daemon rechaza un write-set vacío (`WRITE_SET_EMPTY`, `runtime/src/enforcement:513`) sólo **después** de registrar la cadena del proveedor, así que una tarea `NO_COMMIT` o de sólo lectura no puede llegar a `CHECKPOINTED` y el gasto precede al rechazo (ensayo A de S1; S1 corrió con un write-set de un archivo por fallo del DT). P-16 decide el camino lícito de `NO_COMMIT` —si un write-set vacío es lícito y dónde se rechaza, antes de todo gasto— sin valor por defecto; el vocabulario `READ_ONLY`/`workspaceMode` es de P-17. **Sigue abierta tras A1** (decisión 224): no la lleva A1, que no toca `runtime/src/enforcement`; la lleva un corte posterior de P-16 (D2 en el mapa v3, con N03). **Obligación heredada de P-26/B (D-B-1, decisión 184), pagada por P-16/A1 (decisión 221: `ENVELOPE_VERSION_SUPERSEDED` y `ENVELOPE_VERSION_MISMATCH`, antes de todo gasto, probado por `runDaemonChild`):** `TaskEnvelope.contractVersion` admite sólo la versión vigente y `readRecordedTask` re-parsea el envelope guardado (`runtime/src/recorded-task:148`); tras el bump a 2.10.0, toda tarea ingresada bajo 2.9.0 es ilegible y falla como un parse genérico (`ENVELOPE_UNREADABLE`), no por nombre. Sin historia operativa antes de P9 nada se pierde; P-16/A1, cuyo bump lo repite, lo rechaza por nombre (`ENVELOPE_VERSION_SUPERSEDED` o la palabra que adjudique el DT) antes de cualquier gasto, con su negativo —una tarea ingresada bajo la versión previa, releída, da el rechazo nombrado y cero eventos de proveedor—, y declara la re-sumisión como camino |
...
| P-21 | Daemon residente: cola, concurrencia y reap | `DESIGN_READY`. **Recibido de P-27/A (ADR 0115, decisión 195):** la cohorte V2 de estados de tarea —ningún enum V2 existe en código y el DT no declaró V2 la cohorte de este build (adjudicación ND-P27-1, punto 2)—; hasta entonces `readinessOf` responde `UNKNOWN(TASK_COHORT_LEGACY)` en R1 y `UNKNOWN(DEPENDENCY_COHORT_LEGACY)` en R2, y ningún nodo es READY en producción; el terminal `COMPLETED` (decisión 22) que satisface `WAIT_SUCCESS`; las transiciones de paso fuera de `DECLARED` (con un corte posterior de P-27), sin las cuales un paso `DECLARED` con `dependsOn` responde `UNKNOWN(STEP_DEPENDENCIES_UNPRODUCED)`; **recibido de P-27/C (ADR 0116, decisión 202):** R1 lista también el bloqueo `TASK_LINK_MOVED` —el vínculo actual de la tarea ya no es el paso del grafo, o no tiene ninguno—, que la cohorte V2 consume tal cual: el nodo viejo de una tarea re-vinculada no puede leerse READY; el consumo de `evaluateReady` por el scheduler, que re-verifica revocación y presupuesto antes de despachar (interacción §3.2); y, junto a la cohorte V2, la lectura de efectos de una dependencia: el adaptador lee hasta 500 efectos por arista y hoy trata un listado truncado como `DEPENDENCY_OUTCOME_UNKNOWN` (mal rotulado: el adaptador dejó de leer, no hay un efecto desconocido), inalcanzable en este build porque `DEPENDENCY_COHORT_LEGACY` responde antes, y sin consumidor mientras la cohorte sea legacy | [contratos §2.1](../../architecture/contracts/index.md), [tests §9.4](../../quality/testing/index.md) | P-13, P-14, P-15, P-18/recuperación; límites de cuenta: P-19; concurrencia con gasto: P-34/admisión antes de habilitar | operativa: rutas |
...
| ID | Packet | Diseño | Contratos y schemas | Depende de | Falta congelar |
| --- | --- | --- | --- | --- | --- |
| P-23 | Descriptores de adapter, composición y conformidad | `DESIGN_READY`: H-5 adjudicado | [integraciones §4](../../architecture/integrations/index.md), [composición y documentos de contrato](../../architecture/integrations/composition/index.md) | garantías: P-15 y primitiva compartida de contratos §2.0; composición: desarrollo tras P-15; habilitación M7 tras P-18/recuperación, P-21/P-22 y P-20/P-24 según perfil | operativa: familias/perfiles anunciados, evidencia de interacción y write-set exacto. **Recibido de P-15 (decisión 219):** ND-A2-7, la compuerta de versión del CLI de Claude previa al spawn (ADR 0112 §Tres; hasta que aterrice responde el pin de sha del ejecutable en el kit del operador); las filas negativas del límite declarado de la decisión 214 —`descriptorEnablesWrites(["--permission-mode", "dontAsk"]) === true`, y `auto` y `manual`—, en el primer corte de P-23 cuyo write-set incluya `providers/test/session/index.test.ts`; una captura de 2.1.282 o posterior, sólo si cambia el binario fijado; y `--permission-prompts none` y `--disallowedTools`, sólo con evidencia nueva, en el próximo cambio del adapter Claude |
| P-11 | Error de herramienta nunca es éxito | `DESIGN_READY` | [contratos §4.2](../../architecture/contracts/index.md) | P-01 | operativa: rutas |
| P-24 | Descubrimiento, schema versionado y paginación de herramientas | `DESIGN_READY`; **primer corte (sólo fixtures) entregado: una herramienta se llama sólo bajo el schema con que se permitió — pin `inputSchema` obligatorio comparado por valor, lista paginada y acotada, descubrimiento antes de cada llamada, `SCHEMA_MISMATCH`; ADR 0109; decisiones 161–164.** **Corte (b) (sólo fixtures) entregado: el schema de salida se fija por valor (ausente = ninguno) y `structuredContent` se admite sólo cuando un bloque de texto lo lleva igual por valor; sin espejo → `RESULT_NOT_CARRIED`, conexión conservada; ausente bajo pin → `PROTOCOL_VIOLATION`; nunca se valida contra el schema (desviación declarada); ADR 0117; decisiones 204–207; auditoría Fable pendiente de su cuota.** **Corte (a) (sólo fixtures) entregado: un servidor de herramientas se consulta por las dos puertas —la lectura privada `GET /api/v1/tool-servers/:serverId/tools` tras el bearer y el verbo `acp tool-servers`, que no abre ledger y rechaza `--database`— por un alcance de descubrimiento en el único sitio de composición, sin `callTool`; responde las herramientas permitidas anunciadas bajo su pin, ordenadas por nombre, o la palabra del puerto con su `at` y, en un desajuste, `toolName`; no registra nada, no envía ningún `tools/call` y cosecha el hijo antes de responder; `listTools` prefiere el rechazo de transporte llevado; API 0.24.0 (P-16/A1 tomó la siguiente menor, 0.25.0; decisión 222); paridad CLI = API; ids de 101 a 120 caracteres sólo por la CLI (límite declarado); ADR 0118; decisiones 208–211; auditoría Fable pendiente de su cuota.** Quedan, cada una con su dueño aquí: (c) la ventana B en la composición del daemon con conexiones largas: re-listar por llamada o confiar en la notificación —corte de integración productiva de P-24; si la composición se asigna a P-21, la obligación se mueve con ella y no queda sin dueño—; (d) el perfil real y sus permisos del owner (servidores, binarios, alcance, roles de escritura, credenciales, conformidad viva, exposición al modelo, gasto, egreso de red, captura de frames, revisiones aceptadas, qué puede llevar un resultado al prompt); (e) varios pines aceptados por herramienta —ahora dos schemas por herramienta—, sólo si el owner lo quiere; (f) llevar `structuredContent` al llamador como campo propio (runtime, protocolo, puertas, su propia versión menor de API tomada del HEAD real: 0.24.0 ya es del corte (a)), sólo si un consumidor o el owner lo pide —hacia un prompt es además (d)—; (g) una sola palabra para «resultado conforme que este cliente no lleva»: el bloque de imagen, audio o recurso sigue en `PROTOCOL_VIOLATION` y pasa a `RESULT_NOT_CARRIED` en un corte posterior; (h) la autoría de pines: responder los schemas anunciados para que un operador escriba un pin —entrega bytes no confiables del par a una salida del operador, hasta ~2 MiB por listado; quiere cotas propias y decidir si antes necesita la conformidad viva de (d)—; (i) la prosa operativa fuera del corte (a) que aún dice «una lectura privada»: `docs/operations/runbook.md` y `docs/operations/troubleshooting.md` —corte de documentación posterior de este paquete—; (j) el `HEAD` implícito de las dos lecturas privadas corre el handler tras el bearer —en `toolServerTools` arranca y cosecha un hijo sin registrar nada—; responderle 405 es un cambio del registrador compartido (`registerPrivateGet`) y queda para un corte posterior de este paquete, declarado en ADR 0118 | integraciones fila 6 | P-11; integración productiva: P-06, P-07, P-18/protocolo | operativa: rutas |
...
| ID | Packet | Diseño | Contratos y schemas | Depende de | Falta congelar |
| --- | --- | --- | --- | --- | --- |
| P-26 | Iniciativas, revisiones de roadmap y pasos | **Entregado 2026-09-24 (commits A `1cf47ef`, B `07f4f9c`, C `fd3ba50`; ADR 0110, 0111 y 0113; decisiones 165–170, 172–177, 185–188 y la errata 184 (L-P26B-1); cada corte con receipt independiente y postauditoría Fable), aceptado por decisión 193 el 2026-09-25 en el límite del packet.** A2 se cumple en su límite: revisión inmutable sólo-inserción con autor (`recordedBy`), digest (`contentDigest`) y OCC en la puerta (`expectedHeadDigest` juzgado por `decideRoadmapVersion` bajo `BEGIN IMMEDIATE`), y `(initiative_id, version)` único por restricción (migración 24). A10 se cumple en el límite de P-26: diff semántico por `stepId` de cambios y dependencias (corte C), y restaurar crea una versión `ROLLBACK` nueva con `restoresVersionId` que re-declara los pasos restaurados, nunca reescribe (contrato de P-09; puerta e inserción sólo en el corte A; pasos re-declarados en el corte B); la cláusula de `roles` responde la ausencia nombrada `STEP_ASSIGNMENTS_UNPRODUCED` y su diff de asignaciones por paso es de P-28 (decisión 188), así que A10 cierra en M9 sólo cuando P-28 la reemplace. Lo que P-26 entrega a otros requisitos: a A3 (P-27) las referencias versionadas de paso —`roadmap_step_read_model(roadmap_version_id, step_id)`, `stepId` estable entre versiones, `dependencyRank`— sobre las que P-27 revisa el grafo; a A9 (P-29) las lecturas de versiones y pasos acotadas a una iniciativa y seleccionadas por número; la paginación sigue siendo de P-29. Aceptar el packet no cierra A2, A10 ni M9: la habilitación M9 sigue condicionada por M3, M5, M6 y P-34/admisión, y (f), (g), (h) e (i) siguen con sus dueños. Registro de los cortes: **corte A entregado: la ley de versión de roadmap corre dentro del append (requisito A2) — la puerta de iniciativa decide con `decideRoadmapVersion` bajo su propio `BEGIN IMMEDIATE`, el gateway responde una negativa de la puerta como `WRITE_CONFLICT`, sólo inserción por las dos claves en el pliegue, séptima palabra `VERSION_ID_REUSED`, migración 24 con índice único y preflight sobre el stream; ADR 0110; decisiones 165–170.** **Corte B entregado: una versión declara sus pasos, todo o nada — manifiesto privado `PLAN_DOCUMENT`, un `ROADMAP_STEP_DECLARED` por paso con título, digests y `dependencyRank`, puerta de lote con replay juzgado entero, la puerta re-deriva desde el manifiesto leído por referencia, doce palabras, un productor, migración 25, `CONTRACT_VERSION` 2.10.0 con sus tres actos y API 0.20.0; ADR 0111; decisiones 172–177.** **Corte C entregado: A10, el diff semántico entre versiones — `diffRoadmapVersions` puro sobre filas del modelo de lectura (sin lectura nueva del ledger), dos lecturas API_ONLY seleccionadas por número de versión (`initiativeRoadmapSteps` `?version=`, `initiativeRoadmapDiff` `?from=&to=`), cambios por `stepId` con los campos por nombre, pares de dependencia, `contentChanged`, `restores` por número e id, `roles` como `STEP_ASSIGNMENTS_UNPRODUCED` derivada de las filas, sin digests ni referencias en la lectura de pasos (L-P26C-1) y API 0.21.0; ADR 0113; decisiones 185–188.** Quedan, además ((e) ya no: lo cumplió el corte C): (f) transiciones de estado de paso → P-27; routing STEP → P-28; (g) G-UI: vista de pasos, el timeline que muestra un paso como "Active → Active" y la lista `DECISION_REFUSAL_NAMES` a ampliar con las cinco palabras de paso; (h) un manifiesto publicado y no referenciado tras perder una carrera en la puerta → recolector de P-36 (ND-B7); (i) el ingreso no verifica que `stepId` sea un paso que la versión vinculada declara (deuda nombrada por el ADR 0087 / P-14, decisión 78, a cargo de P-26; implementable desde el corte B, `listRoadmapSteps`, y no pagada en esta aceptación) → **reasignada a P-27 por la decisión 193**, cuya fila carga la obligación. Registro previo al corte B, cumplido por él: (a) **corte B** (ND-6, ND-7): pasos declarados por una puerta de lote de iniciativa todo-o-nada con replay juzgado por lote entero (un replay parcial es una familia de fallo nueva que el brief de B debe nombrar), manifiesto privado `PLAN_DOCUMENT`, `stepId` acotado y estable entre versiones, ciclos rechazados en la puerta, cohorte de `RoadmapVersion` desde 2.10.0, la ubicación del resultado de ciclos de la planificación §4 en la edición de spec de B, y el cuarto tipo de evento como bump de API; la consola mostrando un paso como "Active → Active" es límite declarado bajo G-UI; (b) **corte C**: A10 y el diff por `stepId` — cumplido por el corte C; (c) **decisiones del owner ND-4 y ND-5**: correr la migración 24 sobre un ledger real del operador es autorización del owner, y si su preflight lo rechaza, o su stream tiene un evento de roadmap malformado, la salida es una entrada de excepción o un ledger en cuarentena — la migración nunca borra ni renumera; (d) UI de pasos y diff (G-UI) y un verbo de CLI para escribir roadmap, fuera de API_ONLY; bajo G-UI también: `DECISION_REFUSAL_NAMES` de la consola no incluye `WRITE_CONFLICT` (preexistente: una carrera perdida se muestra como "Refused: unknown." con el mensaje) ni `VERSION_ID_REUSED` (hoy inalcanzable desde el gateway). La habilitación M9 sigue condicionada por M3, M5, M6 y P-34/admisión | hoja de planificación | P-14; habilitación M9: M3, M5, M6; habilitación de equipos: P-34/admisión | operativa: rutas |
| P-27 | DAG, nodos, aristas y predicado READY | **Corte A entregado 2026-09-25 (commit `83952f1`; ADR 0115; decisiones 194–198; receipt independiente y `pnpm check` completo exit 0):** migración 26 con las tres tablas de planificación §5 y el trigger `supersede_once`; dos tipos de iniciativa (`TASK_GRAPH_DECLARED`, `TASK_GRAPH_NODE_DECLARED`) en un lote todo-o-nada por la puerta de lote, con `decideTaskGraph` (siete palabras; un ciclo se rechaza nombrando sus nodos, A3; un nodo cuya tarea no entró en ese paso —otra iniciativa, otro par `(roadmapVersionId, stepId)`, sin vínculo o sin ingreso registrado— se rechaza con `GRAPH_TASK_OUT_OF_SCOPE`, así el paso de una tarea es inequívoco) y el productor `declareTaskGraph`; `evaluateReady` puro y total sobre R1–R4 tri-estado, con el oráculo 3 × 5 como definición de las tres políticas, y `readinessOf` alimentando ausencias nombradas (ningún nodo es READY en producción: cohorte legacy, P-21); la ruta `initiativeStepGraph` (GET y POST, séptima escritura, API 0.22.0); L-P26B-1 ampliada en su lugar, L-P27-1 y L-P27-2. Nada despacha. Siguen de P-27: las transiciones de estado del paso (con P-21), la medición de fan-in y concurrencia de A5 (P-19, P-21), un productor que adopte en un paso una tarea sin vínculo y re-vincule una tarea al paso de una versión posterior del roadmap (hasta entonces una tarea es nodo sólo del paso exacto en que entró; decisión 196; entregado en el corte C, decisiones 200–203), y los tres barridos O(N²) hermanos del rebuild —`foldRoadmapStep` cuenta los pasos de una versión recorriendo `roadmapSteps` (`projection/index.ts:5216`, con su `>=` alcanzable: `STEP_COUNT_MISMATCH` en `candidate.stepIndex`), `assertRoadmapStepsComplete` filtra todos los pasos por versión (`:5260-5263`) y `hasVersionNumber` recorre todas las versiones por versión (`:5536-5539`)—, acotados por versión (200) y medidos en el corte B sin tocarlos (decisión 199; líneas del árbol del corte B). **Obligación recibida por la decisión 193 (era deuda de P-14 a cargo de P-26, ADR 0087, decisión 78), pagada en el corte A (decisión 197):** el ingreso rechaza un `stepId` que la versión vinculada no declara, leyendo los pasos declarados con `listRoadmapSteps(roadmapVersionId)` (`roadmap_step_read_model`, P-26/B), con su propia palabra de rechazo junto a `ROADMAP_VERSION_UNKNOWN` en la lista cerrada y ordenada `TASK_INTAKE_CODES` (clase `REQUEST_INVALID`, como su hermana; `TASK_INTAKE_WRITE_REFUSALS` no se mueve). La palabra mueve `TASK_INTAKE_CODES` y el mensaje `WRITE_REFUSED` del POST de tareas, que la lleva como texto libre; si `API_CONTRACT_VERSION` se mueve lo decide el brief de P-27. Regla de cohorte: una versión anterior al corte B (`stepCount` `null` en el modelo de lectura) no declara nada conocible y el chequeo responde esa cohorte por su nombre, nunca como paso desconocido; una versión 2.10.0 con `stepCount: 0` no declara ninguno y todo `stepId` se rechaza — desconocido no es cero. Pagada con `ROADMAP_STEP_UNKNOWN` (paso ausente, también con `stepCount: 0`) y `ROADMAP_STEPS_UNDECLARED` (cohorte anterior, `stepCount` `null`, rechazo fail-closed: ND-P27-9), en `TASK_INTAKE_CODES` 10 → 12; las palabras viajan con API 0.22.0. **Corte B entregado (decisión 199):** el pliegue del grafo de tareas, lineal en sus eventos —contador por revisión e índice de cabeza por paso, sin cambio observable; la guarda `>=` del contador se conserva, alcanzable tras un rechazo capturado en el cierre—, y un test del CLI que nombra `ROADMAP_STEP_UNKNOWN` y `ROADMAP_STEPS_UNDECLARED`. **Corte C entregado 2026-09-25 (ADR 0116; decisiones 200–203):** una tarea cambia de paso sólo por un `TASK_STEP_LINKED` registrado por la puerta única de iniciativas —adopción de una tarea que entró sin paso a cualquier paso declarado de su iniciativa, o re-vínculo al mismo `stepId` en una versión estrictamente posterior—, con `decideTaskStepLink` (seis palabras) y el productor `linkTaskToStep`; la migración 27 y su tabla insert-only; `currentTaskStepLink` como la única lectura del paso actual, que la puerta de grafos lee para `GRAPH_TASK_OUT_OF_SCOPE`; R1 gana `TASK_LINK_MOVED`; `verifyIntegrity()` informa el primer vínculo y la pertenencia de un nodo en su secuencia, e informa sin lanzar un vínculo que el pliegue rechaza; la ruta `initiativeTaskStep` (GET y POST, octava escritura, API 0.23.0, sin bump de contrato); L-P27C-1. Nada despacha. Siguen de P-27 tras el corte C: las transiciones de estado del paso (con P-21), la medición de A5 (P-19, P-21), los tres barridos O(N²) hermanos (decisión 199) y el residuo de gramática de B1 (iii): un `stepId` con dos puntos, que `BoundedIdentifier` admite y `StepLocalKey` del ingreso no, así que ese paso no recibe tareas por ingreso | [base de datos §7](../../architecture/database/index.md), hoja de planificación | P-26, P-05; despacho: P-19, P-21 | operativa: rutas |
| P-28 | Equipos por paso, aprobaciones, espera humana y adjudicación | `DESIGN_READY`; el diff de A10 (P-26/C) responde `roles` como ausencia nombrada —derivada de las filas— hasta que P-28 produzca asignaciones de scope STEP; P-28 la reemplaza por el diff de asignaciones por paso. **Recibido de P-27/A (ADR 0115, decisión 195):** el productor de aprobaciones de la clase A6 (`owner_approval_read_model`, sujeto `PLAN`/`STEP`/`TASK`), sin el cual R4 responde `UNKNOWN(APPROVAL_UNPRODUCED)`; y la precedencia STEP > INITIATIVE > GLOBAL de la asignación, que hoy R4 lee sólo en GLOBAL, comparando la resolución que registró el ingreso con la lectura GLOBAL vigente (`ASSIGNMENT_UNRESOLVED` cuando ya no coincide); y la re-resolución de una tarea ya ingresada tras una re-publicación del registro, sin la cual un documento de asignación GLOBAL re-publicado —aun con el mismo contenido, otra asignación por identidad— deja sus nodos en `ASSIGNMENT_UNRESOLVED` | contratos §5 y §12, [interacción §3](../../architecture/contracts/interaction/index.md), planning §8.1 | P-27, P-16; equipos activos: P-19, P-21, P-22; habilitación de equipos: P-34/admisión | operativa: rutas y perfil de espera explícito |
...
| ID | Packet | Diseño | Contratos y schemas | Depende de | Falta congelar |
| --- | --- | --- | --- | --- | --- |
| P-36 | Puertos de ledger, artefactos y credenciales, con backup, restore y retención | `DESIGN_READY`. **Recibido de P-26 (ADR 0111, decisión 193; ND-B7):** (h) un manifiesto de pasos publicado en el plano privado y no referenciado tras perder una carrera en la puerta de lote (P-26/B) queda publicado y sin referencia hasta que el recolector de P-36 lo recoja | integraciones filas 8, 9 y 11; base de datos §12 | local: P-09/log, P-18/protocolo; cierre M12: P-10, P-18/recuperación, P-23 | operativa: rutas; segundo backend sólo si se anuncia |
...
| P-37 | Extracciones restantes de estructura y separación de tipos | `DESIGN_READY` | estructura §3 y §5; [arquitectura §7](../../architecture/index.md) | P-13, incremental por seam; cierre M13: P-29, P-36 | operativa: mapa de paths contra HEAD. **Deuda heredada nombrada por P-15/F (ADR 0107):** el `Sha256Hex` privado de `protocol/src/schemas` duplica el `Sha256Hex` de contracts (18 usos en la base, +3 de P-15/F, más la declaración); plegarlo poniendo el `Sha256Hex` de contracts en su barrel y borrando la copia de protocol; `EffectIdParam` pasa entonces a ser alias del de contracts. **Seam pagado (decisión 171):** `Sha256Hex` está en los dos barrels de contracts, protocol no declara ni reexporta ninguno, `EffectIdParam` es alias del de contracts, y L-P37S-1 impide una copia en `protocol/src`. **Seam 2 pagado (decisiones 212-213; paga el ND-3 de la 171):** `isSha256Hex(value: unknown)` está en `primitives` de contracts y en sus dos barrels, y lee con `Sha256Hex` una sola gramática privada del módulo; los 16 archivos que guardaban su propio `/^[0-9a-f]{64}$/` (accounts 1, runtime 3, durability 3, daemon 2, ledger 7 —la fila contaba 18 archivos con ledger 8/12; medido en `00f6b5b`: 16 archivos y 22 sitios literales, con `ledger/src/projection` que el primer censo omitió—) lo llaman en 35 sitios, sin otro cambio textual; L-P37S-2 impide una copia nueva en `packages/*/*/src` y L-P36C-3 lee la llamada en vez del texto del regex. **Queda para un seam posterior:** `requireArtifactDigest` (`ledger/src/ledger`) y `requireDigest` (`ledger/src/artifact-plane`) son el mismo ayudante dos veces con el mismo mensaje; plegarlos es un segundo movimiento (nombre, hogar, clase de error). Fuera por diseño: los tres regex de digest de `scripts/check-architecture.mjs` y el `SHA256_HEX` de `scripts/acquire-restate-server.mjs`, las copias de los árboles de test (oráculo independiente) y las gramáticas SQL congeladas de las migraciones |
