...
| P-27 | DAG, nodos, aristas y predicado READY | `DESIGN_READY`; **obligación recibida por la decisión 193 (era deuda de P-14 a cargo de P-26, ADR 0087, decisión 78):** el ingreso rechaza un `stepId` que la versión vinculada no declara, leyendo los pasos declarados con `listRoadmapSteps(roadmapVersionId)` (`roadmap_step_read_model`, P-26/B), con su propia palabra de rechazo junto a `ROADMAP_VERSION_UNKNOWN` en la lista cerrada y ordenada `TASK_INTAKE_CODES` (clase `REQUEST_INVALID`, como su hermana; `TASK_INTAKE_WRITE_REFUSALS` no se mueve). La palabra mueve `TASK_INTAKE_CODES` y el mensaje `WRITE_REFUSED` del POST de tareas, que la lleva como texto libre; si `API_CONTRACT_VERSION` se mueve lo decide el brief de P-27. Regla de cohorte: una versión anterior al corte B (`stepCount` `null` en el modelo de lectura) no declara nada conocible y el chequeo responde esa cohorte por su nombre, nunca como paso desconocido; una versión 2.10.0 con `stepCount: 0` no declara ninguno y todo `stepId` se rechaza — desconocido no es cero | [base de datos §7](../../architecture/database/index.md), hoja de planificación | P-26, P-05; despacho: P-19, P-21 | operativa: rutas |
| P-28 | Equipos por paso, aprobaciones, espera humana y adjudicación | `DESIGN_READY`; el diff de A10 (P-26/C) responde `roles` como ausencia nombrada —derivada de las filas— hasta que P-28 produzca asignaciones de scope STEP; P-28 la reemplaza por el diff de asignaciones por paso | contratos §5 y §12, [interacción §3](../../architecture/contracts/interaction/index.md), planning §8.1 | P-27, P-16; equipos activos: P-19, P-21, P-22; habilitación de equipos: P-34/admisión | operativa: rutas y perfil de espera explícito |
