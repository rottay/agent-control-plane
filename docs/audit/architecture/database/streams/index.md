...
| `content_digest` | TEXT | NOT NULL | — | Digest sha256 del artefacto de contenido (§0 `ck_`), ver [artifacts](../artifacts/index.md). **Errata P-15/R (ADR 0104, decisión 130):** para `MODEL_VERSION`, `ROUTING_ASSIGNMENT_GLOBAL` y `PRICE_TABLE`, cuyo contenido viaja en línea en el evento y no se publica como artefacto, es el sha256 del JSON canónico del `payload`, y la puerta del registro lo verifica; las demás clases conservan el digest del artefacto. |
