...
describe("N8: this packet introduced no state and no event type", () => {
  it("leaves the contract's two closed lists exactly where they were", () => {
    // 25 since P-18/protocolo B, which added `TASK_ATTEMPT_OPENED`. The number
    // moving is not this packet's doing and not a loosening of N8: what N8
    // claims is that *this* packet introduced nothing, and the two names it
    // leans on are still the ones it always leaned on.
