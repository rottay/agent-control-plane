 ...
 *
 * This module holds everything `startDaemon` composes: the seam builders
 * (instruction, switch port, landing, conformance gate, checkpoint source,
 * execution port), both walk forms, and the bounded stop/terminate wrappers.
 ...
 * The architecture fence re-scoped its composition laws from the barrel to
 * this file in the same packet, so no law ever reads an empty site.
 */
