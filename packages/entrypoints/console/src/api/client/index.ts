 ...
 * projects it through the one row model defined in `@acp/protocol`. It
...
/**
 * The account-action request body (G7 D3).
 *
 * Re-exported from `@acp/protocol` rather than restated. It used to be declared
 * here field for field, which is how a client and a server drift about what a
 * request is while both compile — the schema the gateway actually validates
 * against is the protocol's, so it is the only shape worth naming.
 */
import type { AccountActionRequest as AccountActionInput } from "@acp/protocol";
