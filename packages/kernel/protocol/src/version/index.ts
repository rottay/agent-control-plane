 ...
 * carries the reasoning.
 *
 * `0.21.0` → `0.22.0` at P-27 cut A: a step's task graph. One route,
 * `initiativeStepGraph`, the seventh write: POST declares one graph revision for a
 * step and GET reads the step's current revision with each node's READY verdict,
 * computed at read time and never stored, and `evaluatedAt`, the instant it was
 * computed against. The contract's `INITIATIVE_EVENT_TYPES` gains
 * `TASK_GRAPH_DECLARED` and `TASK_GRAPH_NODE_DECLARED`, so the timeline DTO's type
 * enum widens by derivation. Four schemas arrive (`TaskGraphQuery`,
 * `TaskGraphDeclarationRequest`, `TaskGraphDeclarationResponse`,
 * `TaskGraphResponse`), and the task intake's refusal message gains two words,
 * `ROADMAP_STEPS_UNDECLARED` and `ROADMAP_STEP_UNKNOWN`. Minor: the route surface
