...
describe("the V2 idempotency key", () => {
  ...
  it("admits every supported version and refuses one outside the set", () => {
    ...
    // (ADR 0098), `"2.9.0"` until P-15 escalón C did (ADR 0103), `"2.10.0"`
    // until P-26 cut B did (ADR 0111), and `"2.11.0"` until P-16/A1 did (ADR 0120).
    expect(ControlPlaneEvent.safeParse(event({ contractVersion: "2.12.0" })).success).toBe(false);
...
describe("only the version in force is emitted (ADR 0072's debt, ADR 0076)", () => {
  it("separates what a reader admits from what an issuer stamps", () => {
    ...
    // escalón B did (ADR 0098), `"2.9.0"` until P-15 escalón C did (ADR 0103),
    // `"2.10.0"` until P-26 cut B did (ADR 0111), and `"2.11.0"` until P-16/A1 did
    // (ADR 0120).
    expect(AdmittedContractVersion.safeParse("2.12.0").success).toBe(false);
  });
...
describe("the effect's key grammar belongs to the contract (decision 42)", () => {
  ...
  it("grows the vocabulary by exactly the three types P-18/protocolo C needs", () => {
    // 28 when C landed; D's two occurrences, F's three outbox types and
    // P-32/captura B's two usage types are asserted in their own blocks below, so
    // this one names C's three and the length is B's.
...
describe("two occurrence types, and no bump (P-18/protocolo D, ADR 0077)", () => {
  ...
  it("grows the vocabulary by exactly two same-state passthroughs", () => {
    ...
    // 30 when D landed; F appended its three after these two, and P-32/captura B
    // its two after F's, so the pair is located by name rather than by being last.
    expect(CONTROL_PLANE_EVENT_TYPES).toHaveLength(35);
    expect(new Set(CONTROL_PLANE_EVENT_TYPES).size).toBe(CONTROL_PLANE_EVENT_TYPES.length);
...
describe("three outbox types, a command id grammar and a bump (P-18/protocolo F, ADR 0078)", () => {
  ...
  it("grows the vocabulary by exactly the three names coordination §6.2 fixes", () => {
    ...
    // approval (`:314-315`). 33 when F landed; P-32/captura B appended its two
    // after these three, so the three are the ones just before B's pair.
    expect(CONTROL_PLANE_EVENT_TYPES).toHaveLength(35);
    expect(new Set(CONTROL_PLANE_EVENT_TYPES).size).toBe(CONTROL_PLANE_EVENT_TYPES.length);
    expect(CONTROL_PLANE_EVENT_TYPES.slice(-5, -2)).toEqual(OUTBOX_TYPES);
...
describe("the artifact record lands without a bump (P-36/local A, ADR 0081)", () => {
  it("adds no event type to the task stream and moves no version", () => {
    ...
    // names of `CONTROL_PLANE_EVENT_TYPES` do not move for them (33 then; 35 since
    // P-32/captura B's two usage types, which are task events). And the DT's ruling
...
describe("usage is a declared stream and a measured observation, and the version moves (P-32/captura B, ADR 0089)", () => {
  const USAGE_TYPES = ["USAGE_STREAM_DECLARED", "USAGE_OBSERVATION_RECORDED"];

  it("N-P32B-30: grows the vocabulary by exactly two same-state passthroughs, appended last", () => {
    // Two and not one (adjudication Q1): the declaration is the durable fact that
    // keeps a restarted adapter from inventing a new generation for a report
    // already recorded, so it lands before the first report and cannot ride one.
    expect(CONTROL_PLANE_EVENT_TYPES).toHaveLength(35);
    expect(new Set(CONTROL_PLANE_EVENT_TYPES).size).toBe(CONTROL_PLANE_EVENT_TYPES.length);
    expect(CONTROL_PLANE_EVENT_TYPES.slice(-2)).toEqual(USAGE_TYPES);
    // The legacy total is untouched, and nothing is extended from it.
    expect(CONTROL_PLANE_EVENT_TYPES).toContain("TOKEN_USAGE_RECORDED");
    for (const type of USAGE_TYPES) {
      expect(
        ControlPlaneEvent.safeParse(event({ type, fromState: "DISCOVERED", toState: "DISCOVERED" })).success,
        type,
      ).toBe(true);
    }
  });

  it("puts 2.6.0 in force, keeps 2.2.0 through 2.5.0 readable and refuses to issue them", () => {
    ...
    // Read, and no longer issued, since P-07 escalón B put 2.8.0 in force (ADR 0098);
    // and 2.8.0 likewise since P-15 escalón C put 2.9.0 in force (ADR 0103).
  ...
  it("N-P32B-28: still refuses a transcript or a credential smuggled into a usage payload", () => {
    // The contract's guards run for every type; the ledger's closed payload is a
    // second line, not a replacement for these.
    for (const type of USAGE_TYPES) {
      expect(
        ControlPlaneEvent.safeParse(
          event({ type, fromState: "DISCOVERED", toState: "DISCOVERED", payload: { completion: "the model said this" } }),
        ).success,
        type,
      ).toBe(false);
      expect(
        ControlPlaneEvent.safeParse(
          event({ type, fromState: "DISCOVERED", toState: "DISCOVERED", payload: { apiKey: "sk-" + "x".repeat(24) } }),
        ).success,
        type,
      ).toBe(false);
    }
...
describe("the version moves for the result cohort (P-07 escalón B, ADR 0098)", () => {
  it("puts 2.8.0 in force, keeps 2.2.0 through 2.7.0 readable and refuses to issue them", () => {
    // ADR 0084's reason: a cohort, not an identity. Migration 22 keys the effect's
    ...
    // P-15 escalón C moved the literal on to 2.9.0 (ADR 0103): 2.8.0 is now read
    // and no longer issued.
    expect(AdmittedContractVersion.safeParse("2.8.0").success).toBe(false);
    expect(TaskEnvelope.safeParse(envelope({ contractVersion: "2.7.0" })).success).toBe(false);
  });
});

describe("the version moves for the dispatch pin cohort (P-15 escalón C, ADR 0103)", () => {
  it("puts 2.9.0 in force, keeps 2.2.0 through 2.8.0 readable and refuses to issue them", () => {
    ...
    // P-16/A1 moved the literal on to 2.11.0 (ADR 0120): 2.10.0 is now read and no
    // longer issued.
    expect(AdmittedContractVersion.safeParse("2.10.0").success).toBe(false);
    expect(AdmittedContractVersion.safeParse("2.11.0").success).toBe(true);
    expect(TaskEnvelope.safeParse(envelope({ contractVersion: "2.8.0" })).success).toBe(false);
  });
});

describe("the envelope states its instruction once, and the version moves for an identity (P-16/A1, ADR 0120)", () => {
  it("P1: an envelope without `objective` parses, and the schema declares no such key", () => {
    expect(TaskEnvelope.safeParse(envelope()).success).toBe(true);
    expect(Object.keys(TaskEnvelope.shape)).not.toContain("objective");
    expect(Object.keys(TaskEnvelope.shape)).toContain("content");
  });

  it("N1: the old legal shape, `objective` equal to the first text block, is an unknown key at the envelope", () => {
    const parsed = TaskEnvelope.safeParse(
      envelope({ objective: "Freeze the runtime contracts and the mechanical git fence." }),
    );
    expect(parsed.success).toBe(false);
    expect(parsed.error?.issues.map((issue) => ({ code: issue.code, path: issue.path, keys: (issue as { keys?: unknown }).keys }))).toEqual([
      { code: "unrecognized_keys", path: [], keys: ["objective"] },
    ]);
  });

  it("N2: an `objective` that disagrees with the content is refused the same way, and no issue names the retired path", () => {
    const parsed = TaskEnvelope.safeParse(envelope({ objective: "Something the content does not say." }));
    expect(parsed.success).toBe(false);
    expect(parsed.error?.issues.map((issue) => ({ code: issue.code, path: issue.path, keys: (issue as { keys?: unknown }).keys }))).toEqual([
      { code: "unrecognized_keys", path: [], keys: ["objective"] },
    ]);
    expect(parsed.error?.issues.some((issue) => issue.path.join(".") === "objective")).toBe(false);
  });

  it("pins 2.11.0 in force and ten readable members, 2.10.0 among them for ever", () => {
    expect(CONTRACT_VERSION).toBe("2.11.0");
    expect(SUPPORTED_CONTRACT_VERSIONS).toHaveLength(10);
    expect(SUPPORTED_CONTRACT_VERSIONS).toContain("2.10.0");
    expect(SUPPORTED_CONTRACT_VERSIONS.at(-1)).toBe(CONTRACT_VERSION);
    expect(ControlPlaneEvent.safeParse(event({ contractVersion: "2.10.0" })).success).toBe(true);
  });

  it("holds each of the three admission shapes at the boundary: 2.10.0 refused, 2.11.0 admitted", () => {
    expect(TaskEnvelope.safeParse(envelope({ contractVersion: "2.11.0" })).success).toBe(true);
    expect(TaskEnvelope.safeParse(envelope({ contractVersion: "2.10.0" })).success).toBe(false);
    expect(WorkerSlot.safeParse(slot({ contractVersion: "2.11.0" })).success).toBe(true);
    expect(WorkerSlot.safeParse(slot({ contractVersion: "2.10.0" })).success).toBe(false);
    expect(CommitAuthorizationReceipt.safeParse(receipt({ contractVersion: "2.11.0" })).success).toBe(true);
    expect(CommitAuthorizationReceipt.safeParse(receipt({ contractVersion: "2.10.0" })).success).toBe(false);
  });
});
