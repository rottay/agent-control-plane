...
describe("adoption is impossible from here", () => {
  ...
  it("the fence refuses the load command appearing in code", () => {
    const status = withMutation("packages/daemon/src/launchd/render/index.ts", (source) =>
      source.replace(
  ...
  it("the fence refuses the agent directory appearing in test code", () => {
    ...
    const status = withMutation("packages/daemon/test/launchd/render/index.test.ts", (source) =>
      source.replace(
        "const TEMPLATE = ",
  ...
  it("the fence refuses a node import in the pure denylist reader", () => {
    const status = withMutation("packages/daemon/src/launchd/validate/index.ts", (source) =>
