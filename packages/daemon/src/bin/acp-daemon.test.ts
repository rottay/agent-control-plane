...
import { CONFIG_MAX_BYTES, checkConfigPath, loadDaemonConfig } from "./config-file.js";
import { EXIT_CONFIG_CONTENT, EXIT_CONFIG_PATH, EXIT_USAGE, runPackagedEntry } from "./acp-daemon.js";

const HERE = resolve(fileURLToPath(import.meta.url), "..");
const PACKAGE_ROOT = dirname(dirname(HERE));
const REPO_ROOT = resolve(PACKAGE_ROOT, "..", "..");
const BUILT_ENTRY = join(PACKAGE_ROOT, "dist", "bin", "acp-daemon.js");
...
describe("the built artifact", () => {
  ...
  it("keeps the portable shebang in the tracked source", () => {
    ...
    const source = readFileSync(join(HERE, "acp-daemon.ts"), "utf8");
